 ...
 * @Last Modified time: 2026-05-03 21:05:45
*/
...
// 2026-05-03: corretto bug che non permetteva di salvare il tipo di posizione
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($name === '') {
    ...
    if ($stmt->fetch()) {
      ...
      try {
        $stmt = $pdo->prepare("INSERT INTO locations (name, description, type, locale_id) VALUES (?, ?, ?, ?)");
        $stmt->execute([$name, $description, $type, $locale_id]);
  ...
  <form method="post" class="card shadow-sm p-4">
    ...
    <div class="mb-3">
      ...
      <select name="type" class="form-select" required>
        ...
        <option value="scaffale" <?= (isset($_POST['type']) && $_POST['type'] == 'scaffale') ? 'selected' : '' ?>> Scaffale</option>
        <option value="cassettiera" <?= (isset($_POST['type']) && $_POST['type'] == 'cassettiera') ? 'selected' : '' ?>> Cassettiera</option>
        <option value="scatola" <?= (isset($_POST['type']) && $_POST['type'] == 'scatola') ? 'selected' : '' ?>> Scatola</option>
        <option value="altro" <?= (isset($_POST['type']) && $_POST['type'] == 'altro') ? 'selected' : '' ?>> Altro</option>
        <option value="valigetta" <?= (isset($_POST['type']) && $_POST['type'] == 'valigetta') ? 'selected' : '' ?>> Valigetta</option>
      </select>
    </div>
